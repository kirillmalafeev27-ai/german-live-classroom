...
// node_modules/@elevenlabs/client/dist/utils/assert.js
...
var ConnectionError = class _ConnectionError extends LivekitError {
...
var E2EEManager = class extends eventsExports.EventEmitter {
  ...
  setupEventListeners(room, keyProvider) {
    ...
    room.on(RoomEvent.ConnectionStateChanged, (state2) => {
      if (state2 === ConnectionState.Connected) {
            ...
            this.ws.closed.then((closeInfo) => {
              ...
              if (closeInfo.closeCode !== 1e3) {
                ...
                if (this.state === SignalConnectionState.CONNECTED) {
                  this.handleOnClose((_a2 = closeInfo.reason) !== null && _a2 !== void 0 ? _a2 : "Unexpected WS error");
                }
              }
              return;
            }).catch((reason) => {
  ...
  constructor(rtcConfig, mode, loggerOptions) {
    ...
    this.updateState = () => {
      ...
      if (previousState !== this.state) {
        ...
        (_a2 = this.onStateChange) === null || _a2 === void 0 ? void 0 : _a2.call(this, this.state, this.publisher.getConnectionState(), (_b = this.subscriber) === null || _b === void 0 ? void 0 : _b.getConnectionState());
      }
    };
      ...
      next: () => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
          ...
          if (done) {
            ...
            let decodedResult;
            try {
              decodedResult = decoder2.decode(value.content);
            } catch (err) {
  ...
  publishOrRepublishTrack(track_1, options_1) {
    return __awaiter(this, arguments, void 0, function(track, options) {
      ...
      return (function* () {
        ...
        let defaultConstraints2;
        if (track instanceof MediaStreamTrack) {
          defaultConstraints2 = track.getConstraints();
        } else {
          defaultConstraints2 = track.constraints;
          ...
          if (deviceKind && _this2.activeDeviceMap.has(deviceKind)) {
            defaultConstraints2 = Object.assign(Object.assign({}, defaultConstraints2), {
        ...
        if (track instanceof MediaStreamTrack) {
          switch (track.kind) {
            case "audio":
              track = new LocalAudioTrack(track, defaultConstraints2, true, _this2.audioContext, {
                loggerName: _this2.roomOptions.loggerName,
                loggerContextCb: () => _this2.logContext
              });
              break;
            case "video":
              track = new LocalVideoTrack(track, defaultConstraints2, true, {
                loggerName: _this2.roomOptions.loggerName,
                loggerContextCb: () => _this2.logContext
              });
              break;
...
var Room = class _Room extends eventsExports.EventEmitter {
  ...
  constructor(options) {
    ...
    this.connect = (url, token, opts) => __awaiter(this, void 0, void 0, function* () {
      ...
      if (((_a2 = this.regionUrlProvider) === null || _a2 === void 0 ? void 0 : _a2.getServerUrl().toString()) !== ensureTrailingSlash(url)) {
        ...
        return (function* () {
          ...
          try {
            ...
            if (_this2.state === ConnectionState.Connecting || _this2.state === ConnectionState.Reconnecting || _this2.isResuming) {
              ...
              (_a2 = _this2.abortController) === null || _a2 === void 0 ? void 0 : _a2.abort(msg);
              (_c2 = (_b2 = _this2.connectFuture) === null || _b2 === void 0 ? void 0 : _b2.reject) === null || _c2 === void 0 ? void 0 : _c2.call(_b2, ConnectionError.cancelled("Client initiated disconnect"));
...
function constructOverrides(config) {
  ...
  if (config.overrides) {
    overridesEvent.conversation_config_override = {
      ...
      ...config.overrides.asr?.keywords !== void 0 ? { asr: { keywords: config.overrides.asr.keywords } } : {},
      conversation: {
...
var MIN_VOICE_FREQUENCY = 100;
var MAX_VOICE_FREQUENCY = 8e3;
...
// node_modules/@elevenlabs/client/dist/WebRTCAudioAdapter.js
var audioAdapterFactory;
function setWebRTCAudioAdapterFactory(factory) {
...
var AUDIO_VOLUME_THRESHOLD = 0.01;
function convertWssToHttps(origin) {
  return origin.replace(/^wss:\/\//, "https://");
}
var WebRTCConnection = class _WebRTCConnection extends BaseConnection {
  conversationId;
  inputFormat;
  outputFormat;
  room;
  ...
  outputDeviceId = null;
  audioAdapter;
  inputAnalyser = void 0;
  inputVolumeProvider = NO_VOLUME;
  outputAnalyser = void 0;
  ...
  static async create(config) {
    ...
    if ("conversationToken" in config && config.conversationToken) {
      ...
      try {
        const { name: source, version: version2 } = sourceInfo;
        const configOrigin = config.origin ?? HTTPS_API_ORIGIN;
        const origin = convertWssToHttps(configOrigin);
        let url = `${origin}/v1/convai/conversation/token?agent_id=${config.agentId}&source=${source}&version=${version2}`;
    ...
    try {
      ...
      const connection = new _WebRTCConnection(room, conversationId, inputFormat, outputFormat, config);
  ...
  setupRoomEventListeners() {
    ...
    this.room.on(RoomEvent.ConnectionStateChanged, (state2) => {
      if (state2 === ConnectionState.Disconnected) {
        this.isConnected = false;
        this.disconnect({
          reason: "error",
          message: `LiveKit connection state changed to ${state2}`,
  ...
  async setupAudioCapture(track) {
    ...
    try {
      ...
      const result = await this.audioAdapter.setupOutputAnalysis(track, this.outputFormat, onAudioData);
...
function setupWebRTCSession(connection) {
  if (!(connection instanceof WebRTCConnection)) {
    throw new Error(`setupWebRTCSession requires a WebRTCConnection. Received: ${connection?.constructor?.name ?? typeof connection}`);
  }
  return {
    connection,
...
// node_modules/@elevenlabs/client/dist/platform/web/createWorkletModuleLoader.js
...
// node_modules/@elevenlabs/client/dist/platform/web/audioConcatProcessor.generated.js
...
// node_modules/@elevenlabs/client/dist/platform/web/addLibsamplerateModule.js
...
// node_modules/@elevenlabs/client/dist/platform/web/volumeProvider.js
...
// node_modules/@elevenlabs/client/dist/platform/web/compatibility.js
...
function maybePrimeIosPlayback({ sampleRate, format, worklet, audioElement }) {
  ...
  const primeFrameCount = Math.floor(sampleRate * PRIME_DURATION_MS / 1e3);
...
var MediaDeviceOutput = class _MediaDeviceOutput {
  ...
  static async create({ sampleRate, format, outputDeviceId, workletPaths, libsampleratePath, audioContext }) {
    ...
    try {
      ...
      const newOutput = new _MediaDeviceOutput(context, analyser, gain, worklet, audioElement);
      return newOutput;
    } catch (error) {
...
// node_modules/@elevenlabs/client/dist/platform/web/rawAudioProcessor.generated.js
...
// node_modules/@elevenlabs/client/dist/InputController.js
var DEFAULT_INPUT_CHUNK_DURATION_MS = 25;
...
var MediaDeviceInput = class _MediaDeviceInput {
  context;
  analyser;
  worklet;
  inputStream;
  ...
  permissions;
  onError;
  static async create({ sampleRate, format, preferHeadphonesForIosDevices, inputDeviceId, workletPaths, libsampleratePath, onError, inputChunkDurationMs = DEFAULT_INPUT_CHUNK_DURATION_MS }) {
    let context = null;
    ...
    try {
      ...
      if (inputDeviceId) {
        options.deviceId = _MediaDeviceInput.getDeviceIdConstraint(inputDeviceId);
      }
      const supportsSampleRateConstraint = navigator.mediaDevices.getSupportedConstraints().sampleRate;
      ...
      return new _MediaDeviceInput(context, analyser, worklet, inputStream, source, permissions, onError);
    } catch (error) {
...
var WebSocketConnection = class _WebSocketConnection extends BaseConnection {
  socket;
  conversationId;
  inputFormat;
  outputFormat;
  ...
  constructor(socket, conversationId, inputFormat, outputFormat) {
    ...
    this.socket.addEventListener("close", (event) => {
      ...
      this.disconnect(closeCode === 1e3 ? { reason: "agent", context, closeCode, closeReason } : {
        reason: "error",
  ...
  static async create(config) {
    ...
    try {
      ...
      const { name: source, version: version2 } = sourceInfo;
      if (config.signedUrl) {
        const separator2 = config.signedUrl.includes("?") ? "&" : "?";
        url = `${config.signedUrl}${separator2}source=${source}&version=${version2}`;
      } else {
        url = `${origin}${WSS_API_PATHNAME}${config.agentId}&source=${source}&version=${version2}`;
      }
      if (config.environment) {
        url += `&environment=${encodeURIComponent(config.environment)}`;
      }
      ...
      return new _WebSocketConnection(socket, conversation_id, inputFormat, outputFormat);
    } catch (error) {
      socket?.close();
      throw error;
    }
...
// node_modules/@elevenlabs/client/dist/platform/web/audioUnlock.js
var STASH_TTL_MS = 3e4;
var UNLOCK_EVENTS = ["touchstart", "touchend", "click"];
var stashedAudioContext = null;
var discardTimer = null;
var unlockListenerInstalled = false;
...
function installIosAudioUnlockListener() {
  if (!isIosDevice() || unlockListenerInstalled || typeof document === "undefined") {
    return;
  }
...
// node_modules/@elevenlabs/client/dist/platform/web/VoiceSessionSetup.js
...
// node_modules/@elevenlabs/client/dist/platform/web/webAudioAdapter.js
...
// node_modules/@elevenlabs/client/dist/scribe/microphone.js
var microphoneSetup = null;
function setScribeMicrophoneSetup(setup) {
  microphoneSetup = setup;
}
function getScribeMicrophoneSetup() {
  if (!microphoneSetup) {
    throw new Error("No Scribe microphone implementation registered. Import '@elevenlabs/client/platform/web' or provide a custom implementation via setScribeMicrophoneSetup().");
  }
  return microphoneSetup;
}

// node_modules/@elevenlabs/client/dist/platform/web/scribeAudioProcessor.generated.js
...
// node_modules/@elevenlabs/client/dist/platform/web/scribeMicrophone.js
var TARGET_SAMPLE_RATE = 16e3;
var webScribeMicrophoneSetup = async (config, onAudioData) => {
...
var RealtimeConnection = class {
  ...
  getMediaStreamTrackForMute(action) {
    if (!this._mediaStreamTrack) {
      throw new Error(`Cannot ${action} audio without an active microphone MediaStreamTrack. mute() and unmute() are only supported for microphone connections.`);
    }
    return this._mediaStreamTrack;
  }
  /**
  ...
  setWebSocket(websocket) {
    ...
    this.websocket.addEventListener("message", (event) => {
      try {
        ...
        this.eventEmitter.emit(RealtimeEvents.ERROR, new Error(`Failed to parse message: ${error}`));
      }
    });
  ...
  close() {
    ...
    if (this.websocket) {
      this.websocket.close(1e3, "User ended session");
    }
  }
};
...
var ScribeRealtime = class _ScribeRealtime {
  static DEFAULT_BASE_URI = "wss://api.elevenlabs.io";
  static getWebSocketUri(baseUri = _ScribeRealtime.DEFAULT_BASE_URI) {
    return `${baseUri}/v1/speech-to-text/realtime`;
  }
  static buildWebSocketUri(options) {
    const baseUri = _ScribeRealtime.getWebSocketUri(options.baseUri);
    const params = new URLSearchParams();
    params.append("model_id", options.modelId);
    params.append("token", options.token);
    if (options.commitStrategy !== void 0) {
      params.append("commit_strategy", options.commitStrategy);
    }
    if (options.audioFormat !== void 0) {
      params.append("audio_format", options.audioFormat);
    }
    if (options.vadSilenceThresholdSecs !== void 0) {
      if (options.vadSilenceThresholdSecs <= 0.3 || options.vadSilenceThresholdSecs > 3) {
    ...
    if (options.minSpeechDurationMs !== void 0) {
      if (options.minSpeechDurationMs <= 50 || options.minSpeechDurationMs > 2e3) {
    ...
    if (options.minSilenceDurationMs !== void 0) {
      if (options.minSilenceDurationMs <= 50 || options.minSilenceDurationMs > 2e3) {
  ...
  static connect(options) {
    ...
    const sampleRate = "microphone" in options && options.microphone ? 16e3 : options.sampleRate;
    const connection = new RealtimeConnection(sampleRate);
    const uri = _ScribeRealtime.buildWebSocketUri(options);
    const websocket = new WebSocket(uri);
    if ("microphone" in options && options.microphone) {
      websocket.addEventListener("open", () => {
        _ScribeRealtime.streamFromMicrophone(options, connection);
      });
    }
...
function microphoneConstraint(deviceId = "") {
  return deviceId ? { ideal: deviceId } : void 0;
}
async function startMicrophoneMeter({ deviceId = "", fill, value, signal, onDeviceResolved } = {}) {
...
async function startTeacherMicrophone() {
  ...
  try {
    ...
    const keyterms = (getLesson()?.vocabulary || []).map((item) => String(item).replace(/[.…]/g, "").trim()).filter((item) => item && item.length <= 20).slice(0, 50);
    const connection = ScribeRealtime.connect({
      token: result.token,
      modelId: "scribe_v2_realtime",
      languageCode: "de",
